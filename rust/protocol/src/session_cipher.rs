...
use crate::{
    ...
    RatchetKeyStore, Result, SessionRecord, SessionStore, SignalMessage, SignalProtocolError,
...
pub async fn message_encrypt<R: Rng + CryptoRng>(
    ...
    let msg_keys_hash = bytes_to_hex(message_keys.cipher_key(), message_keys.iv())?;
    ...
    let my_receiver_address = bytes_to_hex(
        sender_ephemeral_private.serialize(),
        receiver_ratchet_key.serialize(),
    )?;
...
async fn get_or_create_chain_key<R: Rng + CryptoRng>(
    ...
    if let Some(chain) = state.get_receiver_chain_key(their_ephemeral)? {
        log::warn!("{remote_address} has existing receiver chain.");
        let our_ephemeral = ratchet_key_store
            .load_ratchet_key(hex::encode(their_ephemeral.serialize()))
            .await?;
...
fn bytes_to_hex(v1: impl AsRef<[u8]>, v2: impl AsRef<[u8]>) -> Result<String> {
    let mut v1 = hex::encode(v1);
    let v2 = hex::encode(v2);
    v1.push('-');
    v1.push_str(&v2);
    Ok(v1)
}
